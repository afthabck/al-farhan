        ...
        <div class="card-body">
          ...
          <div class="table-responsive">
            <table id="vueDataTable" class="table display nowrap" style="width:100%">
              ...
              <tbody>
                <tr v-for="(row, index) in props.machines" :key="index">
                <td>{{ row.name }}</td>
                <td>{{ row.status == 1 ? "Active" : "Inactive" }}</td>
                <td>
                    <button class="btn btn-sm btn-info me-2" @click="editMachine(row.id)" title="Edit">
                        <i data-feather="edit"></i>
                    </button>
                    <button class="btn btn-sm btn-danger" @click="deleteMachine(row.id)" title="Delete">
